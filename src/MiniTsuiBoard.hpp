...
public:
    ...
    int is_linear_check(int pos);//線駒での王手判定
    int is_close_check(int pos);//隣からの王手判定
    int is_check(int pos);//王手判定
    int board[41];//7,8,9,10,11/13,14,15,16,17,18/20,21,22,23,24/26,27,28,29,30/32,33,34,35,36
    int hands[10];//歩~飛,先後
    int bcheck[3];//全、隣、線
    int kpos[2];//歩~飛,先後
